...
class DashboardView:
    ...
    def _content(self, content):
        ...
        ttk.Button(top, text="DATA 업로드", command=a.open_file, style="Modern.TButton").pack(side="left", padx=(0, 6))
        ttk.Button(top, text="DATA 다운로드", command=a.download_uploaded_data, style="Modern.TButton").pack(side="left", padx=(0, 12))
        ...
        for fallback_icon, title, color, filename in kpi_specs:
            card_shell=tk.Frame(a.kpi_frame,bg="#EEF6FF",bd=0);card_shell.pack(side="left",fill="x",expand=True,padx=6,pady=(0,3))
            card=tk.Canvas(card_shell,bg="#EEF6FF",highlightthickness=0,height=82,bd=0);card.pack(fill="both",expand=True)
        ...
        a.top_title=tk.Label(a.top_title_frame,text="1. 월별 클레임 발생현황",font=("Malgun Gothic",18,"bold"),bg="white",fg="#102A4C");a.top_title.pack(side="left");a.month_title_frame=tk.Frame(a.top_frame,bg="white");a.month_title_frame.pack(anchor="w");a.top_canvas=tk.Canvas(a.top_frame,bg="white",height=430,highlightthickness=0);a.top_canvas.pack(fill="x",expand=True);a.top_scroll=ttk.Scrollbar(a.top_frame,orient="horizontal",command=a.top_canvas.xview);a.top_scroll.pack(fill="x");a.top_canvas.configure(xscrollcommand=a.top_scroll.set);a.fixed_table=tk.Canvas(a.top_frame,bg="white",highlightthickness=0,width=105,height=125);a.bottom_canvas=tk.Canvas(a.dashboard_tab,bg="#EEF6FF",highlightthickness=0);a.bottom_canvas.grid(row=1,column=0,sticky="nsew");a.bottom_canvas.bind("<Configure>",lambda e:a._schedule_render());a.canvas=a.bottom_canvas
        detail_frame=ttk.Frame(a.detail_tab);detail_frame.pack(fill="both",expand=True);detail_frame.rowconfigure(0,weight=1);detail_frame.columnconfigure(0,weight=1);a.tree=ttk.Treeview(detail_frame,show="headings");a.tree.grid(row=0,column=0,sticky="nsew");detail_v=ttk.Scrollbar(detail_frame,orient="vertical",command=a.tree.yview);detail_h=ttk.Scrollbar(detail_frame,orient="horizontal",command=a.tree.xview);detail_v.grid(row=0,column=1,sticky="ns");detail_h.grid(row=1,column=0,sticky="ew");a.tree.configure(yscrollcommand=detail_v.set,xscrollcommand=detail_h.set)
